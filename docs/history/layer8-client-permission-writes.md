...
## 채택 재검토 실측 — pnpm 설치 경로와 저작 처방의 공백 (2026-08-19)

채택 아크(#14~#18) 전체를 제3 세션이 독립 재검토("이렇게까지 변경할 일인가")
하는 과정에서, 아크가 측정하지 않은 경로 두 개를 실측했다. 재검토의 결론
자체는 채택 유지였다 — 중간 지대 부재(마스크·force 전멸 실측)와 고장
비대칭(개방: 드물지만 형태 무한·진단 고가·0000 복구 불능 / 차단: 잦지만
형태 단일·문서화·스윕 가능·복구 한 줄)이 근거.

- **맥발 신규 생성 파일은 서버 644로 착지한다.** 스크래치 파일 생성 실측:
  생성 644, 맥 `chmod +x` exit 0에 서버 무변화, 실행 126. 층 8 채택 실측이
  "기존 파일 chmod 무효"까지만 갔던 것을 "신규 생성 경로도 동일"로 확장.
- **맥발 `pnpm install`은 성공하되 실행 불능 체인을 만든다.** 마운트 위
  스크래치 패키지에 pnpm 10.33 `add semver`: 설치 exit 0, `.bin` shim 서버
  644 일반 파일, 직접 실행 126, `pnpm exec`는 `spawn semver EACCES`.
  기존 트리(채택 전 설치분, 8/4 실측 755)는 실모드 보존으로 계속 동작 —
  **파열은 채택 시점이 아니라 다음 신규 설치에서 온다.** 아크의 "17개 레포
  전부 이상 없음" 검증이 이 경로를 못 본 이유다.
- **처방 실증 3종.** ① 게스트 chmod +x 후 shim 정상 실행(rc 0). 셔뱅·ELF·
  Mach-O 매직 스윕 명령을 스크래치에 대해 검증(4종 파일 — shim/가짜 ELF/
  가짜 Mach-O는 +x, plain은 불변). ② 맥발 `ln -s` 성공 — 서버는 에뮬레이션
  일반 파일(1067B)로 저장하나 맥에서 통과·실행·쓰기 관통 모두 동작. APFS
  로컬 디렉토리로 node_modules를 빼는 경로 성립. ③ 게스트 설치는 기각 —
  pnpm 부재(corepack 0.35는 있음)에 더해, 결정적으로 플랫폼 네이티브
  패키지가 linux-arm64로 해석되어 맥이 결과물을 실행할 수 없다.
- **저작 처방은 2단계임을 명시.** `git update-index --chmod=+x`는 커밋
  모드만 고친다 — 워크트리 실파일은 여전히 644라 게스트 chmod가 한 번 더
  필요하다. 그리고 저작 누락은 index=worktree=644라 드리프트 스윕에
  안 걸린다 — doctor에 셔뱅-vs-인덱스모드 스윕(WARN)을 추가한 근거.
  사전 측정에서 기존 9건 검출(스킬 파이썬 스크립트 8건 + sourced 라이브러리
  1건 — 모두 인터프리터 호출/소싱용이라 의도된 644로 판단, 운영자 확인
  대상으로 남김).
