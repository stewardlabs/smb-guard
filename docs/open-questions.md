...
층 7 의 무기한 stale 은 상시가 아니라 **상태**다 — 같은 설정·프로세스·세션에서 병리
상태(단발 읽기 무기한 stale)와 건강 상태(notify push 로 40ms 무효화)가 모두 실측됐다.
무엇이 병리 상태로 밀어 넣는지 모른다. 후보는 맥 sleep/wake 의 세션 재개, 게스트 시계
step(층 1), 클라이언트 notify 구독 유실이며, inotify 큐 오버플로는 5만 이벤트 폭풍으로
재유발에 실패해 단독으로는 기각에 가깝다.

`nodatacache` 처방(층 7)은 이 트리거와 무관하게 내용 일관성을 보장하므로, 이 항목은
처방의 전제가 아니라 **모델의 완결성** 문제다.

재개 조건: `nodatacache` 적용 상태에서 stale 이 다시 관측되면 즉시 재개한다 — 그것은
처방의 반증이기도 하다. 병리 상태를 만나면 층 7 의 채집 절차를 **reload 전에** 수행할 것.

### `nodatacache` 실효의 병리 상태 대비 미검증

`nodatacache` 의 무결성 실측(모든 stale 패턴 ≤0.05s)은 병리 상태가 소멸한 뒤(건강
상태)의 것이다. 병리 상태에서의 실효는 기전 논증(데이터 캐시가 없으면 stale 할 캐시도
없다)에 의존하며 직접 실측되지 않았다 — 병리 상태를 재현할 방법이 없었기 때문이다
(위 항목). 논증이 견고해 채택했으나, 위 재개 조건이 발동하면 이 항목이 반증된 것이다.
