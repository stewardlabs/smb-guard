...
### 기전 — lease 는 깨지지 않고, 치유는 notify 가 한다

- 맥 클라이언트는 읽은 파일에 SMB2 lease `(RH)` 를 받아 쥔다. **게스트 로컬 쓰기 후에도
  lease 는 그대로다** (`smbstatus --locks` 로 쓰기 전후 대조). break 를 보낼 주체가
  없으니 당연하다.
- 그런데도 치유가 일어날 때의 주역은 `kernel change notify` (Samba 기본 yes, inotify
  기반)다. notifyd 는 **서버 로컬 쓰기도 inotify 로 본다.** 이 push 가 살아 있으면
  열린 fd 재읽기조차 40ms 안에 신선해진다.
- `kernel change notify = no` 로 끄면 폴링 치유가 무기한으로 퇴화하는 것을 확인했다
  (동일 프로토콜에서 fast/무기한 교대 관측 — 교대 자체의 원인은 미해명이나 방향은 명확).
- 디렉터리 열거(`ls`)만은 notify 유무와 무관하게 항상 신선하다(0.01s) — 열거는 캐시를
  경유하지 않고 서버 왕복한다.

### 쌍안정 — 같은 설정에서 두 상태가 관측됐다

같은 smb.conf, 같은 smbd 프로세스(PID 동일), 같은 SMB 세션에서 두 상태가 모두 관측됐다:

- **병리 상태**: 단발·held-fd·부분읽기 무기한 stale. 25분간 일관 재현.
- **건강 상태**: 위 전부가 ≤2s. lease 를 쥔 채로도 신선하다 — notify push 가 무효화를
  밀어준다.

전환은 `smbcontrol all reload-config` 2회와 시간적으로 일치했다 (인과 미확증). inotify
큐 오버플로(큐 한계 16384 의 3배인 5만 이벤트 폭풍)로 병리 상태를 재유발하려 했으나
실패했다. **웨지 트리거는 미해명이다** — [open-questions.md](open-questions.md) 참조.

병리 상태를 다시 만나면 **고치기 전에 채집하라** (원칙 22 — 자기 치유를 먼저 멈춰라.
`reload-config` 가 병리를 지울 수 있다):
...
autofs 맵의 마운트 옵션에 `nodatacache` 를 추가한다 ([install.md](install.md)). 데이터
캐시를 끄면 내용 읽기가 항상 서버에 닿으므로, **notify push 의 생사와 무관하게** 내용
일관성이 성립한다 — 검증한 세 후보 중 push 채널에 의존하지 않는 유일한 처방이다.

비용은 실측으로 0 이다 (기본↔옵션 A-B-B-A 교차, 각 3회 중앙값):
...
이런 작업 부하는 메타데이터 왕복이 지배하고, 데이터 캐시가 기여하던 것은 "같은 내용을
다시 읽기"뿐이라 소스·문서 작업에서 잃는 것이 측정되지 않는다. 남는 것은 메타데이터
stale(stat 의 크기·mtime, 상한 ~29s)인데, mtime 민감 도구(빌드)는 게스트에서 돌므로
수용한다. 단발 재읽기·held-fd·부분읽기 전 패턴이 이 옵션 하나로 0.05s 이내가 되는 것을
확인했다.
